...
@interface PGScrollNavigationBar () <UIGestureRecognizerDelegate>
...
@implementation PGScrollNavigationBar
...
{
    self.scrollState = PGScrollNavigationBarStateNone;
...
- (void)handlePan:(UIPanGestureRecognizer*)gesture
{
    ...
    if (deltaY < 0.0f) {
        self.scrollState = PGScrollNavigationBarStateScrollingDown;
    } else if (deltaY > 0.0f) {
        self.scrollState = PGScrollNavigationBarStateScrollingUp;
    }
    ...
    bool isScrolling = (self.scrollState == PGScrollNavigationBarStateScrollingUp ||
                        self.scrollState == PGScrollNavigationBarStateScrollingDown);
    ...
    if (isScrolling && !gestureIsActive) {
        ...
        if (self.scrollState == PGScrollNavigationBarStateScrollingDown) {
            frame.origin.y = maxY;
            alpha = 1.0f;
        }
        else if (self.scrollState == PGScrollNavigationBarStateScrollingUp) {
            frame.origin.y = minY;
            alpha = kNearZero;
        }
...
{
    if (animated) {
        [UIView beginAnimations:@"PGScrollNavigationBarAnimation" context:nil];
    }
...
@implementation UINavigationController (PGScrollNavigationBarAdditions)

- (PGScrollNavigationBar *)scrollNavigationBar
{
    return (PGScrollNavigationBar *)self.navigationBar;
}
