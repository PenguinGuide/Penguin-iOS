...
- (void)_baseConfigs {
    ...
    [self.segmentView.allButton addTarget:self action:@selector(segmentControlDidChangedValue:) forControlEvents:UIControlEventTouchUpInside];
    [self.segmentView.goodsButton addTarget:self action:@selector(segmentControlDidChangedValue:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)segmentControlDidChangedValue:(UIButton *)sender {
    // remove obsever
    ...
    if (index == 0) {
        self.segmentView.allButton.selected = YES;
        self.segmentView.goodsButton.selected = NO;
    } else {
        self.segmentView.allButton.selected = NO;
        self.segmentView.goodsButton.selected = YES;
    }
    UIViewController<ARSegmentControllerDelegate> *controller =
