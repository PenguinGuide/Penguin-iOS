...
- (void)setCellWithGood:(PGGood *)good
{
    ...
    [attrS addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithHexString:@"AFAFAF"] range:NSMakeRange(good.discountPrice.length+2, good.originalPrice.length)];
...
- (UILabel *)titleLabel {
	if(_titleLabel == nil) {
		_titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, self.goodImageView.bottom, 160-40, 30)];
...
- (UILabel *)priceLabel {
	if(_priceLabel == nil) {
		_priceLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, self.titleLabel.bottom+3, 160-40, 16)];
	}
	return _priceLabel;
}
